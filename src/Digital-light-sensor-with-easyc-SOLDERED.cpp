 ...
 */
void LTR507::initializeNative() { pinMode(pin, INPUT); }
...
uint16_t LTR507::getLightIntensity() {
  ...
  readRegister(LIGHT_INTENSITY_ADDRESS, raw,
               sizeof(uint8_t) *
                   2); // Get two bytes that consist of light intensity data
...
bool LTR507::Available() {
  return !(
      sendAddress(PART_ID_REG)); // checking if address is succesfully sent, if
